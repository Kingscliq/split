...
async function readWalletBalances(token: string, wallet: string): Promise<WalletBalanceState> {
  ...
  const xlm =
    token === TOKEN_CONTRACTS.XLM ? asset : await getTokenBalance(TOKEN_CONTRACTS.XLM, wallet);
  return { asset, xlm };
}
...
export default function SplitDetailPage() {
  ...
  const loadWalletBalance = useCallback(async () => {
    ...
    try {
      ...
      const message =
        caught instanceof Error ? caught.message : "Could not read this wallet’s balance.";
      if (/no Testnet XLM|not found|does not exist/i.test(message))
        setWalletBalance({ asset: 0n, xlm: 0n });
      else {
        setWalletBalance(null);
  ...
  const visibleParticipants = useMemo(
    () => (ownShare ? [ownShare] : participants),
    [ownShare, participants],
  );
  ...
  async function pay() {
    ...
    if (!payer) {
      if (connectionIssue) {
        setPayIssue({
          kind:
            connectionIssue.code === "missing"
              ? "wallet_missing"
              : connectionIssue.code === "wrong_network"
                ? "wrong_network"
                : "transaction",
    ...
    if (!share)
      return setPayIssue({
        ...
        message:
          "This wallet is not assigned a share in this Split. Connect the participant wallet named by the organizer.",
      });
    ...
    try {
      ...
      if (xlmBalance <= 0n) {
        setPayIssue({
          kind: "funding",
          message: "This wallet has no Testnet XLM yet. Fund it with Friendbot before paying.",
        });
        return;
      }
  ...
  function shareOnWhatsApp() {
    ...
    const message = encodeURIComponent(
      `Pay your share for ${split?.title ?? "this split"}: ${url}`,
    );
  ...
  const canViewSplit =
    ...
    participants.some((participant) => participant.participant === address);
  if (!canViewSplit)
    return (
      <AppShell>
        <div className="contract-state detail-state error-state">
  ...
  const balanceShortfall =
    walletBalance && remainingShare > walletBalance.asset
    ...
    <AppShell>
      <div className="detail-heading">
        ...
        <div className="detail-actions">
          ...
          {address === split.creator && split.status === "Active" && (
            <button
              type="button"
              onClick={() => void close()}
          ...
          {closeError && (
            <span className="detail-action-error" role="alert">
              {closeError}
            </span>
          )}
        </div>
      </div>
      {(transactionReceipt ?? queryReceipt) && (
        <TransactionReceipt {...(transactionReceipt ?? queryReceipt)!} />
      )}
      ...
      <section className="detail-hero lime-card">
        ...
        <div className="detail-amount">
          ...
          <strong>
            <i>{symbol === "USDC" ? "$" : "✦"}</i>
            {formatAmount(split.totalPaid)} <small>{symbol}</small>
          </strong>
          <p>
            of {formatAmount(split.totalAmount)} {symbol}
          </p>
        </div>
        ...
        <div className="detail-metrics">
          ...
          <div>
            <span>{ownShare ? "Your share" : "Participants"}</span>
            <strong>
              {ownShare ? `${formatAmount(ownShare.amountOwed)} ${symbol}` : split.participantCount}
            </strong>
          </div>
        </div>
      </section>
      ...
      <div className="detail-grid">
        <section className="people-card">
          <div className="section-heading compact">
            <div>
              <p className="eyebrow">{ownShare ? "Your payment" : "The group"}</p>
              <h2>{ownShare ? "Your assigned share" : "Who has paid?"}</h2>
            </div>
            <span className="pill pill-muted">
              {ownShare ? ownShare.status : `${paidCount} of ${participants.length} paid`}
            </span>
          </div>
          <div className="status-list">
            {visibleParticipants.map((person, index) => (
              <div
                className={`status-row ${person.participant === address ? "current-wallet" : ""}`}
              ...
              >
                ...
                <div className="status-amount">
                  <strong>
                    {formatAmount(person.amountPaid)} / {formatAmount(person.amountOwed)}
                  </strong>
                  <span>{symbol}</span>
                </div>
        ...
        <aside className="pay-card">
          <div>
            <span className="mini-kicker">{ownShare ? "Your turn" : "Direct payment"}</span>
            <h2>
              {ownShare
            ...
            <Link className="pay-guide-link" href="/onboarding">
              First time here? Read the Testnet guide →
            </Link>
          </div>
          <div className="pay-amount">
            <span>{symbol === "USDC" ? "$" : "✦"}</span>
            <strong>{formatAmount(remainingShare)}</strong>
            <small>{symbol}</small>
          </div>
          {address && (
            <div
              className={`wallet-balance-card${balanceShortfall > 0n ? " insufficient" : ""}`}
              aria-live="polite"
            >
              ...
              {balanceLoading && !walletBalance ? (
                <strong>Checking Testnet…</strong>
              ) : walletBalance ? (
                <>
                  <strong>
                    {formatAmount(walletBalance.asset, 4)} <small>{symbol}</small>
                  </strong>
                  {ownShare && remainingShare > 0n && (
                    <p>
                      {balanceShortfall > 0n
                        ? `You need ${formatAmount(balanceShortfall)} more ${symbol} to pay this share.`
                        : "You have enough for this share."}
                    </p>
                  )}
                  {symbol !== "XLM" && (
                    <small>
                      {formatAmount(walletBalance.xlm, 4)} XLM available for network fees
                    </small>
                  )}
                  {walletBalance.xlm <= 0n && (
                    <Link href="/onboarding#fund-testnet-wallet">Fund this Testnet wallet →</Link>
                  )}
                </>
              ) : (
                <>
                  <strong>Balance unavailable</strong>
                  <p>{balanceError ?? "Refresh to try again."}</p>
                </>
              )}
            </div>
          )}
          ...
          {payIssue && (
            <div className={`pay-action-error ${payIssue.kind}`} role="alert">
              <strong>
              ...
              <div>
                ...
                {payIssue.kind === "funding" && (
                  <Link href="/onboarding#fund-testnet-wallet">Fund wallet with Friendbot →</Link>
                )}
      ...
      <section className="share-strip">
        <div>
          ...
          <div>
            <strong>Bring everyone in</strong>
            <p>Share this split with the group.</p>
          </div>
        </div>
        <div className="share-buttons">
          ...
          <button
            className="whatsapp"
            type="button"
            aria-label="Share this split on WhatsApp"
