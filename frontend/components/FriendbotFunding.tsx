...
function fundingError(responseBody: string) {
  ...
  if (normalized.includes("already") || normalized.includes("exist") || normalized.includes("balance")) {
...
export function FriendbotFunding() {
  ...
  async function fundWallet() {
    ...
    if (!testnetAddress) {
      setState({
        kind: "error",
        message: connectionError ?? "Could not connect Freighter. Check the wallet message above, then try again.",
        source: "wallet",
      });
      return;
    }
    ...
    try {
      ...
      setState({
        kind: "error",
        message: caught instanceof Error ? caught.message : "Friendbot could not fund this wallet. Try again shortly.",
        source: "friendbot",
      });
    }
  }
    ...
    <article className="guide-step friendbot-card" id="fund-testnet-wallet" aria-labelledby="friendbot-title">
      ...
      <div className="friendbot-content">
        <div className="friendbot-copy">
          <span className="friendbot-mark" aria-hidden="true">✦</span>
          <div>
        ...
        <div className="friendbot-action">
          ...
          <button className="button button-dark" type="button" onClick={() => void fundWallet()} disabled={busy || state.kind === "success"}>
      ...
      {state.kind === "success" && (
        <div className="friendbot-status success" role="status">
          <span className="friendbot-status-icon" aria-hidden="true">✓</span>
          <span>Test XLM was sent to {shortAddress(state.address)}. Open Freighter to confirm the Testnet balance.</span>
        </div>
      )}
      {state.kind === "error" && (
        <div className="friendbot-status error" role="alert">
          <span className="friendbot-status-icon" aria-hidden="true">!</span>
          <span className="friendbot-status-copy"><strong>{state.source === "wallet" ? "Wallet connection issue" : "Funding issue"}</strong><span>{state.message}</span></span>
        </div>
      )}
    </article>
  );
